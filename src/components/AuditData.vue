<template>
  <div>
    <x-header :right-options="{showMore: true}" @on-click-more="showMenus = true" class="header">订单核销</x-header>
      ...
      <cell title="管理员账号:huaqingchi" style="border:none;background: white;height: 40px;">
        <div class="badge-value">
        <span class="fcolor">订单记录</span>
        </div>
      </cell>
      <br>
      <grid :cols="2" :show-lr-borders="false" :show-vertical-dividers="false">
        <grid-item >
          <span class="grid-center">待发货订单</span>
          <span class="grid-center">0</span>
        </grid-item>
        <grid-item >
          <span class="grid-center">待自提订单</span>
          <span class="grid-center">0</span>
        </grid-item>
      </grid>
      <br>
      <cell title="销售数据" style="border:none;background: white;">
      </cell>
      <tab v-model="index01" prevent-default @on-before-index-change="switchTabItem">
        <tab-item>今日</tab-item>
        <tab-item>昨日</tab-item>
        <tab-item selected>近7日</tab-item>
        <tab-item>近30日</tab-item>
      </tab>
    <grid :cols="2" :show-lr-borders="false" :show-vertical-dividers="false">
      <grid-item >
        <span class="grid-center">下单金额</span>
        <span class="grid-center">0</span>
      </grid-item>
      <grid-item >
        <span class="grid-center">下单金额</span>
...
import { Badge, Group, Cell, XTextarea, CellBox, ViewBox, XButton, Flexbox, FlexboxItem } from 'vux'
